...
from vset_utils.vset_select import get_mediamanager, get_videosets, get_timeseries_options
    ...
    def get_frame_at_timestamp(self, timestamp):
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Unable to retrieve frame for timestamp {timestamp}: {e}")
...
def route_timestamps():
    ...
    try:
        ...
        return jsonify({"timestamps": ts, "count": len(ts)})
    except Exception as e:
        traceback.print_exc()
        return error_response(str(e), 500)
